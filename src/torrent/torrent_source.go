...
func (s *torrentSource) Probe(ctx context.Context) (media.MediaInfo, error) {
	...
	defer reader.Close()
	if info, err := media.Probe(ctx, reader); err == nil {
		return info, nil
	} else if ctx.Err() != nil {
	...
	return media.ProbeURL(ctx, s.url)
}
...
func parseMagnet(magnet string) (*torrent.TorrentSpec, string, error) {
	...
	if hash.IsZero() {
		if spec.InfoHashV2.Ok {
			return nil, "", ErrUnsupportedMagnetVersion
		}
