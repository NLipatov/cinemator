...
func TestGenerateVideoWindowUsesAbsoluteTimelineAndMappedWebVTT(t *testing.T) {
	...
	direct, err := GenerateDirectWindow(
		...
		StreamSelection{AudioTrackIndex: 0},
		1, 7, 1, 6*time.Second, 30*time.Second,
	...
	if !strings.HasPrefix(direct.Fragments[0].Name, "direct_000007_") {
		t.Fatalf("direct asset %q reused source segment namespace", direct.Fragments[0].Name)
	}
	...
	unknownDirect, err := GenerateDirectWindow(
		...
		StreamSelection{AudioTrackIndex: 0},
		0, 0, 5, 6*time.Second, 30*time.Second, nil,
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	hybrid, err := GenerateDirectWindow(
		...
		StreamSelection{AudioTrackIndex: 0},
		0, 0, 2, 6*time.Second, 30*time.Second, nil,
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGenerateDirectWindowCoalescesFrequentKeyframes(t *testing.T) {
	...
	result, err := GenerateDirectWindow(
		...
		0, 0, 2, 2*time.Second, 30*time.Second, nil,
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := GenerateDirectWindow(
				...
				StreamSelection{SubtitleTrackIndex: -1},
				0, 0, 1, 2*time.Second, 30*time.Second, nil,
			)
			if err != nil {
				t.Fatal(err)
			}
...
func TestAnalyzeTailDurationBeyondSeekWindow(t *testing.T) {
	...
	window, err := GenerateDirectWindow(
		...
		StreamSelection{SubtitleTrackIndex: -1},
		2, 2, 1, 6*time.Second, 30*time.Second, nil,
	)
	if err != nil {
		t.Fatal(err)
	}
