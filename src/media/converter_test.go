...
func TestValidateSelectionRejectsOutOfRangeTracks(t *testing.T) {
	info := MediaInfo{
		...
		Subtitles:   []SubtitleTrack{{Codec: "subrip"}},
	}
	tests := []struct {
		name      string
...
func TestValidateSelectionAllowsDefaultsAndVideoOnlyInput(t *testing.T) {
	tests := []struct {
		...
		{name: "default tracks", info: MediaInfo{}, selection: StreamSelection{AudioTrackIndex: 0, SubtitleTrackIndex: -1}},
		{name: "explicit default audio", info: MediaInfo{AudioTracks: []AudioTrack{{}}}, selection: StreamSelection{AudioTrackIndex: -1, SubtitleTrackIndex: -1}},
	}
