...
| `PLAY-05` | A committed seek has one exact source target. The client recognizes every command boundary the browser exposes for native `<video controls>`: pointer or keyboard intent where available, Chromium's pointer-less `pause → seeking → play` timeline transaction, and a `seeking` event from an already-paused transport outside attach, readiness, or playhead-restoration work. Known internal positioning during attach or restoration is not a user command. Scrub intermediates, late decoded frames from the old position, a trailing media/HLS snap-back, and superseded asynchronous work are cancelled or ignored. | **Gated.** Playwright exercises the exact pointer-less Chromium transport sequence, paused native seeks, pointer/keyboard ownership, internal seek restoration, and snap-back; the deterministic full-stack test physically clicks Chromium's native timeline through consecutive cold forward and backward seeks; Go session tests cover job ownership. |
...
| `PLAY-10` | Once the browser emits an explicit native Pause or Resume command, that intent belongs to the playback session, not to one hls.js attachment. Replacing a presentation never starts a paused session or cancels a requested resume. Before media is playable, control availability and disabled state follow the browser's native `<video controls>` behavior. | **Gated.** Playwright pauses during a cold seek and during a track change, verifies the replacement stays paused, then resumes it explicitly. |

Explicit user commands are Play on another file, a command through the native
video controls, and an audio or subtitle selection change. A media-element seek
event without live native-control intent, library callback, playlist update,
...
| ID | Invariant | Status and evidence |
| --- | --- | --- |
| `MEDIA-01` | A selected text-subtitle track is part of readiness and continuous playback. Startup or a cold seek does not attach video until the subtitle segment covering the target is readable, and does not call `play()` until hls.js has successfully processed that selected subtitle asset and exposed its native text track. Readiness identifies the stable source-segment asset instead of trusting hls.js fragment timing after rendition alignment. Video and subtitle playlists use the same media and discontinuity sequences plus matching program-date-time anchors, so WebVTT remains aligned with the active video timeline after every presentation change. Every later subtitle fragment requested at the active playhead remains foreground work. A cue-free target interval is bounded by that interval's video clock and cannot scan forward through the torrent to a distant subtitle cue. | **Gated.** Fake-engine orchestration, the seeded torrent/real-FFmpeg pipeline, playlist-continuity and program-date-time tests, `TestAddWebVTTTimestampMapKeepsEmptySegmentValid`, `TestGenerateEmptySubtitleSegmentDoesNotReadUntilTheNextCue`, `TestSelectedSubtitleAssetPreemptsBackgroundSessionExecution`, Go status/scheduler tests, real-hls.js coverage, and deterministic full-stack Chromium playback with physical native-timeline clicks, real playlist fetches, readiness ordering, and exact initial, forward-seek, and backward-seek cues exercise the production boundaries. |
| `MEDIA-02` | Selected subtitles are never silently disabled after a load error. Fatal selected-subtitle loss stops playback visibly. | **Covered.** Playwright covers the client policy with mocked HLS errors. A real selected-track loss through the browser text-track pipeline is still **target**. |
...
| `HLS-02` | One source-time model owns segment lookup, duration clamping, canonical windows, and presentation origin. The browser uses the server-reported origin and does not derive a competing mapping. Alternate video and subtitle renditions publish matching media sequences and source-derived program-date-time anchors; client timeline offsets therefore cannot be applied again during rendition alignment. | **Gated.** Pure timeline and playlist tests, Playwright origin/mapped-seek tests, and the real full-stack playlist-alignment checks. |
...
| `WORK-02` | Initial playback, the active playhead, the latest committed seek, and every selected-subtitle fragment requested by the active player are foreground work. Foreground admission preempts obsolete background work before capacity is rejected. A required target subtitle is an admission barrier before later media packaging, while an active-playhead subtitle preempts background video continuation within the same session execution lane. Unrelated encoder and packager lanes remain independent. | **Gated** at scheduler/session boundaries by selected-subtitle, local execution-preemption, and worker-lane independence tests. Long real-source fairness is not yet an SLO gate. |
...
| `WORK-04` | An admitted foreground playback target owns one non-transferable media-worker entitlement until its playable reserve is healthy or the target is explicitly retired. A playback session runs at most one FFmpeg process at a time; its video and subtitle work serialize through a priority-aware session execution lane. Foreground work cancels a background owner and waits for its release before executing, so the two processes never overlap. Source waiting is distinguished from packaging without replacing the foreground process: the live FFmpeg process retains its worker while its torrent reader feeds the missing range. A retry after a real media failure resumes from the first unpublished nominal segment and preserves the complete published prefix. | **Covered** by `TestPackagerWaitsInPlaceForMissingTorrentData`, `TestPlaybackSessionForegroundMediaExecutionPreemptsBackground`, `TestSelectedSubtitleAssetPreemptsBackgroundSessionExecution`, status classification, persistent reservation, per-session serialization, retry-resume, monotonic publication, and scheduler-lane tests. The full controlled slow-source scenario in the QoE target is not yet a CI gate. |
...
| `WORK-06` | Every admitted foreground playback-session target owns one bounded torrent-demand lease for its critical range until the required playable reserve is verified or the target is retired. Repeated canonical demand within that session coalesces. Distinct sessions on the same infohash share one torrent runtime but retain independent HLS presentations, jobs, and ref-counted demand windows; admitting a new session cannot revoke an existing target's worker ownership or piece priority. | **Covered** by playback-session identity, torrent-runtime, piece-demand, and scheduler ownership tests. Bandwidth-aware multi-session admission under constrained peer throughput remains **target**, so a shared swarm may still reduce delivery rate without revoking either target. |
...
| `SAFE-04` | A process has at most one torrent runtime per infohash. One browser playback session keeps a stable stream identity across prepare, seek, recovery, and track work. Different sessions never share mutable presentation or target ownership, even for the same source selection; they share only the torrent runtime and verified piece cache, so a seek or close in one session cannot cancel the other. Leaving the page explicitly releases all presentations owned by that session before the release endpoint completes. | **Covered** by stream-key, API, browser-session, torrent-runtime, synchronous session-release, repeated full-stack lifecycle, and stream-registry tests. A deterministic concurrent multi-browser real-media gate remains **target**. |
| `SAFE-05` | Torrent probing and HLS orchestration do not launch external media processes directly. FFprobe analysis and FFmpeg video, direct-copy, and subtitle packaging cross narrow package-private boundaries, so orchestration, scheduling, publication, retry, and error behavior are deterministic without a real binary. | **Gated.** The architecture test rejects direct process-API calls outside the adapter; fake-engine tests cover video, direct-copy, subtitle, and tail-probe orchestration, while FFmpeg integration tests cover the real adapter behavior. |
...
- Playwright public-behavior coverage for timeline ownership, retained and cold
  ...
  errors, plus a deterministic full-stack Chromium scenario using the real
  Cinemator server, local BitTorrent seeder, FFmpeg/ffprobe, HLS, and WebVTT.
  The full-stack gate physically clicks the native timeline, fetches the live
  video and subtitle playlists, verifies matching sequence/PDT alignment, and
  requires each selected VTT to be processed before the first resumed frame;
...
3. **The deterministic full-stack matrix is narrow.** Chromium now decodes the
   real seeded torrent through FFmpeg, HLS, hls.js, and selected WebVTT across
   consecutive cold seeks, but Safari/native HLS, codec variants, constrained
   network profiles, concurrent viewers, and long playback remain target
   coverage.
...
the measurement loop: persist bounded QoE summaries locally, aggregate the
primary SLIs and zero-tolerance counters, expand controlled full-stack workload
profiles, and make the declared short-run SLOs release-gating.
